...
6. **Аутентификация и авторизация:**
   ...
   - Вход в систему
   - Управление правами доступа (обычный пользователь/администратор)
...
2. **Защита эндпоинтов:**
   ...
   - Функция `admin_required` проверяет роль пользователя из токена
   - При отсутствии прав возвращается статус `403 Forbidden`

3. **Защищенные операции:**
   - Создание, обновление и удаление ресурсов требуют прав администратора
   - Просмотр данных доступен всем авторизованным пользователям
   - Некоторые эндпоинты (списки сотрудников, должностей) доступны без авторизации
...
| GET | `/api/v1/employees` | Вывести список всех работников (с пагинацией) | Нет |
...
| GET | `/api/v1/fired_employees` | Вывести список уволенных работников (с пагинацией) | Нет |
...
**Авторизация:**
- Проверка прав доступа через зависимость `Depends(admin_required)`
- Роли: администратор (role_id=1) и обычный пользователь (role_id=2)
- Операции создания, обновления и удаления требуют прав администратора
