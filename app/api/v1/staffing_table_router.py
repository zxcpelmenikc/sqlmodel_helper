...
@router.get("/staffing_table", tags=["staffing_table"], description= "Вывод расписания с названием должности и департамента")
def get_staffing_with_dept_rel_route(session: Session = Depends(get_session)):
    return gswdr(session)

@router.put("/staffing_table/{id}",tags=["staffing_table"], description= "Изменение расписания")
def update_staffing_info(id: int, data: Staffing_table, session: Session = Depends(get_session), current_user: Users = Depends(admin_required)):
